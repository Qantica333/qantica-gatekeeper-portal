...
const OrbitSystemV3: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const computeMaxRadius = () => (Math.min(orbitPath.offsetWidth, orbitPath.offsetHeight) / 2);
    let MAX_RADIUS = computeMaxRadius();

    const CENTER_BUFFER = () => Math.max(80 * (CONTAINER_SIZE / 600), ITEM_RADIUS * 2 + 10);
    ...
    type ItemObj = {
      ...
      initialX: number;
      initialY: number;
      angle: number;   // ángulo ancla para responsive
      ring: "inner" | "outer"; // anillo ancla
      jitter: number;  // pequeña variación radial
    };

    const NUM_STATIC_ITEMS = staticItemElements.length;
    const angleStep = (2 * Math.PI) / NUM_STATIC_ITEMS;

    // Radios base para dos anillos, proporcionales al contenedor
    const ringRadii = () => {
    ...
    // Inicializamos estáticos con ancla angular/ring (para recolocar en resize sin aleatoriedad nueva)
    const staticItems: ItemObj[] = staticItemElements.map((el, i) => {
      ...
      return {
        ...
        initialX: 0,
        initialY: 0,
        angle: i * angleStep,     // ancla angular estable
        ring,
        jitter: (Math.random() - 0.5) * 16, // +-8px de variación
      };
    });
    ...
    const FRICTION_MOVING = 0.9992; // un poco más baja para tamaños pequeños
    ...
    function positionStaticByAnchor() {
    ...
    function placeYouTube() {
      ...
      do {
        ...
        const radius = START_RADIUS_MIN_YT + Math.random() * (START_RADIUS_MAX_YT - START_RADIUS_MIN_YT);
        x = CENTER_X() + radius * Math.cos(angle);
        y = CENTER_Y() + radius * Math.sin(angle);
        attempts++;
        if (attempts > MAX_ATTEMPTS_YT) break;
      } while (staticItems.some(s => distance(x, y, s.x, s.y) < ITEM_RADIUS * 2) ||
    ...
    // init
    positionStaticByAnchor();
    staticItems.forEach((itemObj) => {
    ...
    const animate = () => {
      // YouTube contra Q
      {
        ...
        x += dx; y += dy;
        youtubeState.x = x; youtubeState.y = y; youtubeState.dx = dx; youtubeState.dy = dy;
        (element as HTMLDivElement).style.left = `${x - ITEM_RADIUS}px`;
        (element as HTMLDivElement).style.top = `${y - ITEM_RADIUS}px`;
      }

      // Estáticos semi-dinámicos con muelle + repulsión
    ...
    // Loop si la pestaña está visible (evita CPU oculta)
    const onVis = () => {
    ...
    resizeObsRef.current = new ResizeObserver(() => {
      ...
      // refresco visual por si el loop está parado
      if (!rafRef.current) {
        rafRef.current = requestAnimationFrame(animate);
      }
    });
    ...
    <div className="orbit-wrapper" style={{ position: 'relative', width: '100%', height: '100%', minHeight: '700px' }}>
      ...
      <div className="container" ref={containerRef}>
        ...
        <div id="content-system">
          ...
          <div className="orbital-item static-item" data-item-id="a24">A24</div>
          ...
          <div className="orbital-item static-item" data-item-id="imdb">IMDB</div>
          <div className="orbital-item static-item" data-item-id="animoca">ANIMOCA BRANDS</div>
          <div className="orbital-item static-item" data-item-id="story">STORY</div>
          <div className="orbital-item static-item" data-item-id="angelstudios">ANGEL STUDIOS</div>
          <div className="orbital-item static-item" data-item-id="patreon">PATREON</div>
          <div className="orbital-item static-item" data-item-id="mubi">MUBI</div>
          <div className="orbital-item static-item" data-item-id="rockstargames">ROCKSTAR GAMES</div>
          <div className="orbital-item static-item" data-item-id="roblox">ROBLOX</div>
        </div>
      </div>
      ...
      <style>{`
        ...
        @keyframes pulseGlow {from{box-shadow:0 0 10px #FFD700,0 0 20px rgba(255,215,0,.5); opacity:.95;} to{box-shadow:0 0 18px #FFD700,0 0 30px rgba(255,215,0,.7); opacity:1;}}
